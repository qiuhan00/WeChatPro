				...
				if(m.isAnnotationPresent(MemCache.class)){
					...
					if(null != memCache){ //如果memcache中存在
						...
						if(null == object || object.equals("null")){
							...
							System.out.println("返回类型："+tempType);
//							object =  JSON.parseArray("["+memresult+"]", (Class) tempType);
//							object = JSON.parse(memresult);
							List<User> obj = JSON.parseArray(memresult, User.class);
							object = obj;
						}
